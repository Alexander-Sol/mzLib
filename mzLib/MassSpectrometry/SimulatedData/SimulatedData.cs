...
namespace MassSpectrometry
{
    ...
    public class SimulatedData
    {
        ...
        public static void AddHighFrequencyNoiseToYArray(double[] inputArray, Normal noiseDistribution)
        {
        ...
        public static MzSpectrum BuildLowFrequencyNoiseSpectrum(LowFrequencyNoiseParameters noiseParams, int? seed = null)
        {
            ...
            double[] xArray = new double[numberOfRandomPeaks];
            double[] yArray = new double[numberOfRandomPeaks];

            for (int i = 0; i < numberOfRandomPeaks; i++)
            {

                xArray[i] = random.NextDouble(noiseParams.PeakLocationLimitLow, noiseParams.PeakLocationLimitHigh);
                yArray[i] = random.NextDouble(noiseParams.PeakIntensityLimitLow, noiseParams.PeakIntensityLimitHigh);

                // TODO: Add support for profile mode noise
            }
            // Because the intensities are randomly generated, we don't need to worry about sorting the y values
            Array.Sort(xArray);
            return new MzSpectrum(xArray, yArray, false);
            ...
            for (int i = 0; i < numberOfRandomPeaks; i++)
            {
                ...
                switch (noiseParams.PeakShapeOptions)
                {
                    ...
                    case PeakShapeOptions.Gaussian:
                        ...
                        break;
                    case PeakShapeOptions.Centroid:

                    case PeakShapeOptions.None:
                        break;
                }
                ElementWiseAddition(Yarray, yArrayToAdd);
                //Yarray = (DoubleArray)Yarray + yArrayToAdd;
            }
        }
        #endregion

        public static void ElementWiseAddition(double[] baseArray, double[] arrayToAdd)
        {
            int minLength = Math.Min(baseArray.Length, arrayToAdd.Length);
            for (int i =0; i < minLength; i++)
            {
                baseArray[i] += arrayToAdd[i];
            }
        }
